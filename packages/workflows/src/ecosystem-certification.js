 ...
 */
export const ECOSYSTEM_CERTIFICATIONS = [
    {
        repo: "notice-respond",
...
export function getEcosystemCertification(repo, workflow) {
    return ECOSYSTEM_CERTIFICATIONS.find((entry) => entry.repo === repo && entry.workflow === workflow);
}
export function isEcosystemExecutable(entry) {
    return entry.status === "executable" || entry.status === "gold";
}
export function isEcosystemGold(entry) {
