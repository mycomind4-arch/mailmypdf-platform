...
export const ECOSYSTEM_CERTIFICATIONS: readonly EcosystemCertification[] = [
  {
    ...
    status: "domain-ready",
    executableCapabilities: ["classification", "extraction", "deadlines", "discrepancies", "evidence", "research", "strategy", "draft", "validation", "review", "approval"],
    blockedBy: ["CP2000 UI still needs explicit approval affordance wired to approveWorkflow", "MailingFunnel calls /api/mail/response but no matching route is registered in routeTree", "deployed provider/path certification"],
    evidence: ["strict sequential runtime gate", "runtime transition regression suite", "CP2000 gold-standard analysis pipeline", "provider submission is fail-closed when endpoint is unavailable"],
  },
  {
    repo: "appeal-mail",
    ...
    blockedBy: ["production submission/tracking/proof certification", "deployed-path verification", "factory is not yet the production workflow execution boundary"],
    evidence: ["factory capability gate", "pack-backed capability resolution", "factory regression tests", "customer UI now derives executable status from runtime pack registration"],
  },
  {
    repo: "dispute-mail",
    ...
    blockedBy: ["actual runtime wiring of approval/submission predicates", "live payment/fulfillment path", "deployed tracking/proof", "remote CI verification"],
    evidence: ["credit-report domain analyzer", "evidence/finding blocking predicates", "consequential submission regression gates", "credit-report UI corrected to fail closed before live integration"],
  },
  {
    repo: "dispute-mail",
  ...
  },
  {
    ...
    engine: "document-action",
    status: "domain-ready",
    executableCapabilities: ["document", "facts", "deadlines", "evidence", "authority", "strategy", "draft", "validation", "review", "approval"],
    blockedBy: ["checkout currently persists a draft mailing order but has no verified Stripe/provider submission path", "production workflow displays a submission success state for the draft-only path unless fail-closed helper is invoked", "deployed fulfillment certification", "remote CI verification"],
    evidence: ["document-understanding", "preflight", "review/approval state model", "checkout false-success identified and blocked at helper boundary"],
  },
  {
  ...
  },
  {
    ...
    blockedBy: ["Gold runner not referenced by production executor", "actual persistence/execution runtime", "MailMyPDF fulfillment integration", "deployed tracking/proof", "live CI verification"],
    evidence: ["evidence-bearing Gold runner", "source-grounded AI analysis worker", "Gold regression tests", "runner usage audit"],
  },
  {
    ...
    blockedBy: ["Gold runner not referenced by production executor", "actual domain runtime wiring", "property/jurisdiction infrastructure", "MailMyPDF fulfillment", "deployed verification"],
    evidence: ["evidence-bearing Gold runner", "Gold regression tests", "runner usage audit"],
  },
  {
