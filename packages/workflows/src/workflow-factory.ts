...
export function composeWorkflow(manifest: WorkflowManifest): WorkflowFactoryResult {
  ...
  try {
    pipeline = getPipeline(manifest.pipeline);
  } catch (error) {
    diagnostics.push({ severity: "error", code: "UNKNOWN_PIPELINE", message: error instanceof Error ? error.message : String(error) });
    pipeline = getPipeline("P01_CORE_MAIL");
  }

  const adapters: ReturnType<typeof getAdapter>[] = [];
  for (const adapterId of manifest.adapters) {
    try {
      adapters.push(getAdapter(adapterId));
    } catch (error) {
  ...
  for (const stage of pipeline.requiredStages) {
    ...
    if (capability && !declared.has(capability)) {
      diagnostics.push({ severity: "error", code: "PIPELINE_CAPABILITY_UNDECLARED", message: `Pipeline ${pipeline.id} requires ${capability}, but workflow ${manifest.id} does not declare it.` });
    }
  }
  ...
  for (const stage of pipeline.requiredStages) {
    const capability = PIPELINE_STAGE_TO_CAPABILITY[stage];
    if (capability && notApplicable.has(capability)) {
      diagnostics.push({ severity: "error", code: "REQUIRED_STAGE_NOT_APPLICABLE", message: `Pipeline ${pipeline.id} requires ${capability}, but workflow ${manifest.id} declares it not applicable.` });
    }
  }

  if (manifest.allowsConsequentialAction && !manifest.requiresHumanReview) {
    diagnostics.push({ severity: "error", code: "MISSING_HUMAN_REVIEW", message: "Consequential workflows require explicit human review." });
  }

  if (manifest.maturity === "production-verified" && diagnostics.some((d) => d.severity === "error")) {
    diagnostics.push({ severity: "error", code: "PRODUCTION_STATUS_INVALID", message: "A production-verified workflow cannot have factory errors." });
  }
...
export function adapterIdsForComposition(manifest: WorkflowManifest): readonly AdapterId[] {
