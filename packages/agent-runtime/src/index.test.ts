...
test('runtime requires approval for non-low-risk approval tasks',async()=>{const runtime=new AgentRuntime();const result=await runtime.run({...task,risk:'HIGH',requiresApproval:true},'search');assert.equal(result.ok,false);assert.match(result.error??'',/approval/i)});
test('capability scoring rewards required and trusted matches',()=>{const result=scoreCapability({id:'n',name:'n',required:['search'],preferred:['voice'],forbidden:[],risk:'LOW'},{id:'c',capabilities:['search','voice'],status:'TRUSTED',risk:'LOW'});assert.ok(Math.abs(result.score-0.8)<Number.EPSILON)});
