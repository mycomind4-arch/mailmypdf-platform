...
test("Docling provider requires HTTPS", () => {
  assert.throws(() => new DoclingHttpProvider({ endpoint: "http://localhost:8080/extract", timeoutMs: 5000 }), /HTTPS/);
});
test("Docling provider rejects unsafe timeout configuration", () => {
  assert.throws(() => new DoclingHttpProvider({ endpoint: "https://docling.example/extract", timeoutMs: 0 }), /timeout/);
});
